...
class T5ForMultitaskLabel(T5ForConditionalGeneration):
    _keys_to_ignore_on_load_missing = [
        r"encoder\.embed_tokens\.weight",
        r"decoder\.embed_tokens\.weight"
        r"lm_head\.0\.weight",
    ...
    def __init__(self, config, loss_type=None, n_layer=None, num_classes=1000):
        super().__init__(config)
        self.config.num_classes = num_classes
        self.set_output_embeddings(nn.Sequential(nn.Linear(config.d_model, num_classes+2)))
        self.config.tie_word_embeddings = False
    ...
    def forward(
        ...
        class_loss = loss_fct1(lm_logits[:,:-2], class_labels.view(-1))
        class_type = torch.argmax(lm_logits, axis=-1)

        loss_fct2 = nn.KLDivLoss(reduction='batchmean')
        yield_label = torch.stack([(100-labels[:,1]), labels[:,1]], dim=1)/100
        yield_pred = nn.LogSoftmax(dim=-1)(lm_logits[:,-2:])
        yield_loss = loss_fct2(yield_pred, yield_label.view(-1,2))
        percent_yield = torch.exp(yield_pred[:,-1])*100

        loss = class_loss + yield_loss
        lm_logits = torch.stack([class_type, percent_yield], dim=1)
        # TODO(thom): Add z_loss https://github.com/tensorflow/mesh/blob/fa19d69eafc9a482aff0b59ddd96b025c0cb207d/mesh_tensorflow/layers.py#L666
